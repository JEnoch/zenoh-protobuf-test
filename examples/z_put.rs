...
async fn main() {
    ...
    let protobuf_shirt = zenoh_protobuf_test::create_large_shirt(value);

    println!("Putting Data ('{}': '{:?}')...", key_expr, protobuf_shirt);
    session
        .put(&key_expr, protobuf_shirt.encode_to_vec())
...
fn parse_args() -> (Config, String, String) {
    let args = App::new("zenoh put example")
        ...
        .arg(Arg::from_usage("-v, --value=[VALUE]      'The shirt color'").default_value("Blue"))
        .arg(Arg::from_usage(
