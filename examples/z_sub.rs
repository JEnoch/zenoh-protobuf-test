...
use zenoh::prelude::*;
use zenoh_protobuf_test::items::Shirt;
    ...
    loop {
        select!(
            sample = subscriber.next() => {
                ...
                let buf = bytes::BytesMut::from(sample.value.payload.contiguous().deref());
                let s: Shirt = Shirt::decode(buf).unwrap();

                println!(">> [Subscriber] Received {} ('{}': '{:?}')",
                    sample.kind, sample.key_expr.as_str(), s);
            },
